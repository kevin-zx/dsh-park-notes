window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* Store one image blob under a caller-chosen id (the caller already shows the
		* in-memory file, so the panel never waits for this round trip).
		* @param id - image id.
		* @param blob - image bytes.
		* @param name - original file name, when known.
		* @returns a promise resolving when the record (with its thumbnail) is stored.
		*/
		function saveImageAs(id, blob, name) {
			var type = blob.type || "image/png";
			return makeThumbBlob(blob, 160).then(function (thumb) {
				return openImagesDb().then(function (db) {
					return new Promise(function (resolve, reject) {
						...
						if (thumb !== null) record.thumb = thumb;
		...
		function ParkNotesDock(props) {
				...
				for (var p = 0; p < pending.length; p++) {
					(function (imageId) {
						loadImage(imageId).then(function (record) {
							...
							blobsRef.current[imageId] = record.blob;
			...
			function addImageFiles(files) {
				...
				for (var j = 0; j < accepted.length; j++) {
					(function (picked) {
						...
						saveImageAs(id, picked, picked.name).catch(function (error) {
			...
			function copyImageToClipboard(imageId, forHandoff) {
				...
				try {
					...
					loadImage(imageId).then(function (record) {
						...
						var prepare = blob.type === "image/png" ? Promise.resolve(blob) : blobToPng(blob);
			...
			*/
			function openPreview(imageId) {
				var full = null;
				try {
					if (blobsRef.current[imageId] !== undefined) full = URL.createObjectURL(blobsRef.current[imageId]);
				} catch (error) {
					full = null;
				}
			...
			*/
			function startImageDrag(event, imageId) {
				try {
					var blob = blobsRef.current[imageId];

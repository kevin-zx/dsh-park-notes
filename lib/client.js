window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css = [
			".parkNotes_dock{box-sizing:border-box;width:100%;max-width:calc(var(--dsh-composer-card-max-width,748px) - 4 * var(--dsh-composer-dock-inset,8px));margin:0 auto;font:inherit;position:relative;z-index:6}",
			...
			".parkNotes_panel{padding:10px 12px;border-top:.5px solid var(--dsw-alias-border-l1);display:flex;flex-direction:column;gap:10px;max-height:min(62vh,560px);overflow-y:auto;overscroll-behavior:contain}",
			".parkNotes_head{display:flex;align-items:center;gap:8px;position:sticky;top:-10px;z-index:3;background:var(--dsw-alias-bg-layer-1);padding:2px 0}",
			...
			".parkNotes_list{display:flex;flex-direction:column;gap:6px;max-height:264px;overflow-y:auto;overscroll-behavior:contain;padding:2px 2px 2px 0;scrollbar-gutter:stable;mask-image:linear-gradient(to bottom,transparent 0,#000 12px,#000 calc(100% - 12px),transparent 100%);-webkit-mask-image:linear-gradient(to bottom,transparent 0,#000 12px,#000 calc(100% - 12px),transparent 100%)}",
			".parkNotes_list::-webkit-scrollbar,.parkNotes_panel::-webkit-scrollbar{width:8px}",
			".parkNotes_list::-webkit-scrollbar-thumb,.parkNotes_panel::-webkit-scrollbar-thumb{background:var(--dsw-alias-border-l2);border-radius:999px}",
			".parkNotes_list::-webkit-scrollbar-thumb:hover,.parkNotes_panel::-webkit-scrollbar-thumb:hover{background:var(--dsw-alias-label-caption)}",
			".parkNotes_list::-webkit-scrollbar-track,.parkNotes_panel::-webkit-scrollbar-track{background:transparent}",
		...
		var en = {
			...
			"autoAttached": "Images attached to the composer automatically",
			"images": "Images",
		...
		/**
		* Session the visible dock belongs to: the `/note` projection has no session
		* of its own, and newer Harness builds no longer expose a `current` field on
		* the session-list snapshot, so the dock (which receives `sessionId` as a
		* standard slot prop) is the reliable source.
		*/
		var visibleSessionId = undefined;
		...
		*/
		...
		/**
		* Locate the composer's rich-text editor: the contenteditable nearest to the
		* dock anchor. The composer owns its paste pipeline, so handing a synthetic
		* paste to that element lets the product attach images with its own code.
		* @param anchor - element to measure against (usually the dock root).
		* @returns the editor element, or null when it cannot be identified.
		*/
		function findComposerEditor(anchor) {
			try {
				if (typeof document === "undefined") return null;
				var nodes = document.querySelectorAll('[contenteditable="true"]');
				if (nodes.length === 0) return null;
				if (nodes.length === 1) return nodes[0];
				var anchorRect = anchor !== null && anchor !== undefined && typeof anchor.getBoundingClientRect === "function"
				...
				for (var i = 0; i < nodes.length; i++) {
					...
					var score = anchorRect === null ? i : Math.abs(rect.top - anchorRect.bottom);
					if (score < bestScore) {
						bestScore = score;
						best = node;
					}
		...
		*/
		function ParkNotesDock(props) {
			...
			// the `/note` projection parks into whichever session last rendered its dock
			if (typeof sessionId === "string" && sessionId !== "") visibleSessionId = sessionId;
			...
			var currentImageIds = useInput(function (s) { return s.imageIds; });
			var imageIdsRef = React.useRef(currentImageIds);
			imageIdsRef.current = currentImageIds;
			...
			/**
			* Best-effort automatic hand-off: hand the images to the composer by
			* dispatching a synthetic paste into its editor, so the product's own paste
			* pipeline attaches them (no clipboard round trip, no manual step).
			* Callers verify the result through the composer's image-id list and fall
			* back to the clipboard when nothing landed.
			* @param ids - stored image ids to attach.
			* @returns a promise of whether the paste event was dispatched.
			*/
			function attachImagesToComposer(ids) {
				var files = [];
				var chain = Promise.resolve();
				var collect = function (imageId) {
					chain = chain.then(function () {
						return resolveImageBlob(imageId, 6).then(function (blob) {
							if (blob === null) return;
							try {
								var type = blob.type || "image/png";
								var ext = type.indexOf("jpeg") >= 0 ? ".jpg" : type.indexOf("webp") >= 0 ? ".webp" : type.indexOf("gif") >= 0 ? ".gif" : ".png";
								files.push(new File([blob], String(imageId) + ext, { type: type }));
							} catch (error) {
								console.error("[park-notes] file rebuild failed:", error);
							}
						});
					});
				};
				for (var i = 0; i < ids.length && i < 4; i++) collect(ids[i]);
				return chain.then(function () {
					if (files.length === 0) return false;
					try {
						var editor = findComposerEditor(dockRef.current);
						if (editor === null) return false;
						var transfer = new DataTransfer();
						for (var j = 0; j < files.length; j++) transfer.items.add(files[j]);
						editor.focus();
						var pasteEvent = new ClipboardEvent("paste", { clipboardData: transfer, bubbles: true, cancelable: true });
						editor.dispatchEvent(pasteEvent);
						return true;
					} catch (error) {
						console.error("[park-notes] synthetic paste failed:", error);
						return false;
					}
				});
			}
			...
			*/
			...
			function pushToDraft(list) {
				...
				for (var k = 0; k < list.length; k++) {
					if (list[k].images === undefined) continue;
					for (var q = 0; q < list[k].images.length; q++) imageIds.push(list[k].images[q]);
				}
				var firstImage = imageIds.length > 0 ? imageIds[0] : null;
				if (body.length > 0) inputActions.setDraft(currentDraft ? currentDraft + "\n\n" + body : body);
				setPushedAt(Date.now());
				if (imageIds.length > 0) {
					// try the automatic hand-off first; verify it through the composer's
					// own image list and only then fall back to the clipboard
					var hadImages = Array.isArray(imageIdsRef.current) ? imageIdsRef.current.length : 0;
					setImageHint(t("preparing"));
					attachImagesToComposer(imageIds).then(function (dispatched) {
						if (!dispatched) {
							copyImageToClipboard(firstImage, true);
							return;
						}
						setTimeout(function () {
							var now = Array.isArray(imageIdsRef.current) ? imageIdsRef.current.length : 0;
							if (now > hadImages) setImageHint(t("autoAttached"));
							else copyImageToClipboard(firstImage, true);
						}, 450);
					});
				}
				var ids = [];
			...
			*/
			...
			return React.createElement("div", { className: "parkNotes_dock", "data-park-notes": "", ref: dockRef },
		...
		* accepted `/note` belongs to it. The dock is authoritative (it receives
		* `sessionId` as a standard slot prop); the session-list snapshot is only a
		* legacy fallback, because newer Harness builds dropped its `current` field.
		* @param ctx - client root context.
		* @returns the session id, or undefined when it cannot be resolved.
		*/
		function activeSessionId(ctx) {
			if (typeof visibleSessionId === "string" && visibleSessionId !== "") return visibleSessionId;
			try {
				var sessions = ctx.get("sessions");
				var state = sessions !== undefined && sessions.list !== undefined && typeof sessions.list.getSnapshot === "function"
					? sessions.list.getSnapshot()
					: undefined;
				if (state !== undefined && state.current !== undefined) return state.current;
			} catch (error) {
				/* fall through */
			}
			return undefined;
					...
					return uiConversation.events.register({
						...
						start: function (context, match) {
							try {
								...
								var target = activeSessionId(ctx);
								if (!trimmed) return { parked: false };
								var parked = parkNoteFromCommand(target, trimmed, data !== undefined ? String(data.commandId) : undefined);
								if (!parked) {
									console.warn("[park-notes] /note could not be parked; session id =", target);
								}

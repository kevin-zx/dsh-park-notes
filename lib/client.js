window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css = [
			...
			".parkNotes_imageHint{color:var(--dsw-alias-state-business-primary);font-size:11px;line-height:18px}"
		].join("\n");
		...
		function ParkNotesDock(props) {
			...
			function addImageFiles(files) {
				...
				var save = function (file) {
					chain = chain
						.then(function () { return saveImage(file, file.name); })
						.then(function (id) {
							setDraftImages(function (current) {
								return current.concat([{ id: id, type: file.type, name: file.name || "" }]);
							});
						});
				};
				for (var j = 0; j < accepted.length; j++) save(accepted[j]);
			...
			function copyImageToClipboard(imageId) {
				try {
					...
					loadImage(imageId).then(function (record) {
						if (record === null || record.blob === undefined) return null;
						var type = record.type || "image/png";
						var item = new ClipboardItem({ [type]: record.blob });
						return navigator.clipboard.write([item]).then(function () { setImageHint(t("copied")); });
			...
			function pushToDraft(list) {
				...
				if (firstImage !== null) copyImageToClipboard(firstImage);
				var ids = [];
			...
			function renderThumbs(note) {
				...
				var buildThumb = function (imageId) {
					...
					thumbs.push(React.createElement("span", { className: "parkNotes_thumb", key: imageId, title: t("previewTitle") },
						url !== undefined
							? React.createElement("img", {
								src: url,
								alt: "",
								onClick: function () { setPreview({ id: imageId, url: url }); }
							})
				...
				body = React.createElement("div", {
					...
					React.createElement("div", { className: "parkNotes_thumbs" },
						draftImages.map(function (image) {
							...
							return React.createElement("span", { className: "parkNotes_thumb", key: image.id, title: t("previewTitle") },
								url !== undefined
									? React.createElement("img", {
										src: url,
										alt: "",
										onClick: function () { setPreview({ id: image.id, url: url }); }
									})
									: React.createElement("span", { className: "parkNotes_meta" }, "\u2026"),
								React.createElement("button", {
									type: "button",
			...
			var lightbox = preview === null ? null : React.createElement("div", {
				...
				React.createElement("div", { className: "parkNotes_lightboxBar" },
					...
					React.createElement("button", {
						type: "button",
						className: "parkNotes_lightboxBtn",
						onClick: function (event) { event.stopPropagation(); setPreview(null); }
					}, t("closePreview"))
				),
